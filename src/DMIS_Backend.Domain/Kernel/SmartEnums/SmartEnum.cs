...
using DMIS_Backend.Domain.Kernel.SmartEnums.Exceptions;
...
{
  /// <summary>
  /// 初始化 SmartEnum 實例（使用 int 作為值類型）
  /// </summary>
  /// <param name="value">列舉值</param>
  /// <param name="name">列舉名稱（如果未提供，將使用 CallerMemberName 自動取得）</param>
  protected SmartEnum(int value, [CallerMemberName] string? name = null) :
...
/// <remarks></remarks>
public abstract partial class SmartEnum<TEnum, TValue> :
    ISmartEnum,
    ...
    where TValue : IEquatable<TValue>, IComparable<TValue>
{
  ...
  /// <param name="name">列舉名稱（如果未提供，將使用 CallerMemberName 自動取得）</param>
  /// <param name="value">列舉值</param>
  protected SmartEnum([CallerMemberName] string? name = null, TValue? value = default)
  {
  ...
  /// <returns>如果物件相等則返回 true，否則返回 false</returns>
